...
7. **Checkpoint tags (create in order):** `step-0-starter` (application + Web MVC), `step-1-rest-dto` (+DTOs/Controller + validation), `step-2-service-db` (+Entity/Repo/Service + H2 + H2 Console), `step-3-complete` (+exception handler + tests + profiles + PostgreSQL), `step-4-outbound-enrichment` (+RestTemplate + proxy-ready external client + composed response). `main` contains the latest completed workshop. Tags may be recreated when the workshop flow changes.
